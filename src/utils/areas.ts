 ...
 * 根据key获取名称
 */
export function getNameByKey(key: string, obj: AreaMap): string {
  const temp = obj[key]
 ...
 * 获取子集
 */
export function getChildren(key1: string, obj: AreaMap): AreaMap {
  const ab = obj[key1]
  if (typeof ab === 'object') {
    for (const key in ab) {
      return ab[key] as AreaMap
    }
  }
  return {}
}
 ...
 * 随机省市区
 */
export function getRandomCountyKey(data: AreaMap): {
  provinceKey: string
  cityKey: string
  countyKey: string
} {
  // Helper function to recursively find a valid county
  function getRandomCounty(obj: AreaMap): string | null {
    ...
    for (let i = 0; i < shuffledKeys.length; i++) {
      ...
      if (typeof value === 'object' && value !== null) {
        const result = getRandomCounty(value)
  ...
  const provinceCode = countyCode.slice(0, 2) + '0000'
